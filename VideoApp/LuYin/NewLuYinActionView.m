...
#import "lame.h"
@interface NewLuYinActionView ()<AVAudioPlayerDelegate,AVAudioRecorderDelegate>{
...
@end
@implementation NewLuYinActionView
...
- (void)viewDidLoad{
    ...
    [super viewDidLoad];
    _background = [[UIImageView alloc] initWithImage:_pic];
    NSLog(@"%@",_pic);
    _background.frame = CGRectMake(0,0, KScreenWidth,KScreenHeight);
    _background.contentMode = UIViewContentModeScaleAspectFill;
    _background.userInteractionEnabled = YES;
//    _blur = [UIBlurEffect effectWithStyle:UIBlurEffectStyleDark];
//    UIVisualEffectView *effectview = [[UIVisualEffectView alloc] initWithEffect:_blur];
...
//    [_background addSubview:effectview];
    ...
    self.cafPathStr = [kSandboxPathStr stringByAppendingPathComponent:[NSString stringWithFormat:@"%@%@.caf",kCafFileName,_index]];
    self.mp3PathStr =  [kSandboxPathStr stringByAppendingPathComponent:[NSString stringWithFormat:@"%@%@.mp3",kCafFileName,_index]];
    [self addHeadView];
    [self addFooterView];
}
#pragma TimerAction
...
#pragma TimeText & listenAgainBUtton
-(void)addHeadView{
    
    
    timeNow=[UILabel labelWithFont:[UIFont systemFontOfSize:36] textColor:[UIColor whiteColor] numberOfLines:0 textAlignment:NSTextAlignmentCenter];
    ...
    [shiTing setImage:[UIImage imageNamed:@"Re_btn_pause_80x18@2x"] forState:UIControlStateNormal];
    [shiTing setImage:[UIImage imageNamed:@"Re2_btn_audition_57x24@2x"] forState:UIControlStateSelected];
...
#pragma BlockSET
-(void)setCallBackVoiceUrl:(void (^)(NSString *))callBackVoiceUrl{
    _callBackVoiceUrl = callBackVoiceUrl;
}
-(void)setCallBackVoiceTime:(void (^)(NSString *))callBackVoiceTime{
    _callBackVoiceTime = callBackVoiceTime;
}
#pragma mark - 弹窗提示
...
-(void)saveAction{
    [self stopRecordNotice];
    if(self.callBackVoiceUrl){
        self.callBackVoiceUrl(self.mp3PathStr);
    }
    if(self.callBackVoiceTime){
        self.callBackVoiceTime([NSString stringWithFormat:@"%ld",(long)_countNum]);
    }
    [self stopPlayRecord];
    [_sliderTimer setFireDate:[NSDate distantFuture]];
    shiTing.selected=NO;
    ...
    seconds = 0;
    shiTing.hidden = NO;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"已保存" message:nil preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
        [self dismissViewControllerAnimated:YES completion:nil];
    }];
    
    UIAlertAction* noAction = [UIAlertAction actionWithTitle:@"试听" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * action){
