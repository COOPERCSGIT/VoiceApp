...
//        _processSlider = [[UISlider alloc] initWithFrame:CGRectMake(0, 500, 200, 20)];
...
-(void)setReurl:(NSString *)reurl{
    _reurl = reurl;
    NSLog(@"PLAY!!!%@",_reurl);
    [self playMusicWithUrl:[NSURL URLWithString:_reurl]];
}
...
- (void)hide
{
    ...
    [_processSlider removeFromSuperview];
...
//    [_aPlayer dealloc];
    ...
    _aPlayer = [[AVPlayer alloc] initWithPlayerItem:item];
    [_aPlayer replaceCurrentItemWithPlayerItem:item];
    AVAsset *currentPlayerAsset = _aPlayer.currentItem.asset;
